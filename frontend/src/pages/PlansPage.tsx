...
export function PlansPage() {
  ...
  const handlePurchase = async (plan: MembershipPlan) => {
    ...
    try {
      const result = await api.purchases.create({ plan_id: plan.id, card_token: 'tok_visa' })
      setSuccess(`Successfully purchased ${plan.name}! Transaction: ${result.transaction_id}`)
      await refresh()
    } catch (err) {
  ...
  return (
    <div>
      ...
      {success && (
        <div className="bg-green-900/40 border border-green-700 text-green-300 px-4 py-2 rounded-lg text-sm mb-4">{success}</div>
      )}

      {membership && (
        <div className="bg-primary/10 border border-primary/30 rounded-xl p-4 mb-6">
          <p className="text-sm text-primary">
            Current plan: <strong>{membership.plan.name}</strong> — {membership.remaining_days} days remaining
          </p>
        </div>
      )}
        ...
        {data?.plans.map(plan => {
          ...
          return (
            <div key={plan.id} className={`rounded-xl border p-6 ${
              isCurrent ? 'border-primary bg-gray-900' : 'border-gray-700 bg-gray-900'
            }`}>
              <h2 className="text-xl font-bold mb-1">{plan.name}</h2>
              <p className="text-3xl font-bold text-primary mb-2">{formatPrice(plan.price_cents)}</p>
              ...
              <div className="space-y-1 mb-6">
                {['learning', 'conversation', 'analysis'].map(f => (
                  <div key={f} className={`text-sm flex items-center gap-2 ${
                    plan.features.includes(f) ? 'text-green-400' : 'text-gray-600'
                  }`}>
